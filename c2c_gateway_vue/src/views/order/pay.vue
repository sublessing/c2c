<template>
  <div class="app-container">
    ...
    <el-table-column type="expand">
      <template slot-scope="props">
        <el-form label-position="left" inline class="demo-table-expand">
          ...
          <el-form-item label="订单状态确认:">
            ...
            <el-button type="primary" size="mini" @click="handleCallbackBatch(props.row)" v-if="urls.includes('/order/pay/callback/') && props.row.notify_merchant !== 0 && props.row.status === 0">回调</el-button>
      ...
      <el-table-column label="充值渠道名" width="100" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="通道回调状态" width="100" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="回调状态" width="100" align="center">
        <template slot-scope="scope">
          <el-tag size="mini" :type="scope.row.notify_merchant | callbackFilter"> {{callbackStatusMap[scope.row.notify_merchant]}}</el-tag>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form class="small-space" :model="form" ref="form" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="支付状态" prop="confirm_type">
          <el-select v-model="form.confirm_type" size="small" clearable placeholder="请选择支付状态">
            <el-option v-for="(value, key) in statusMap1" :key="key" :label="value" :value="key">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
import { listSelectChannel, listOrderRecharge, orderRechargeExcel, orderCallback, orderRechargeToSuccess, orderRechargeQueryMore } from '@/api/service'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      statusMap1: { '0': '支付成功', '1': '支付失败' },
      statusMap: { '0': '支付成功', '1': '支付失败', '2': '待支付', '3': '初始化', '4': '请求支付失败' },
      ...
      orderRechargeExcel(this.queryParams).then(response => {
        ...
        require.ensure([], () => {
          ...
          const tHeader = [
            ...
            '用户实付金额(元)', '手续费', '支付状态', '通道回调状态', '回调状态', '商户回调地址',
          ...
          const filterVal = [
            ...
            'order_real_money', 'pay_fee', 'status', 'way_callback', 'notify_merchant', 'notify_url',
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => {
        if (j === 'channel_code') {
          ...
          return this.callbackStatusMap[''+v.way_callback]
        } else if (j === 'notify_merchant') {
          return this.callbackStatusMap[''+v.notify_merchant]
        } else {
          return v[j]
        }
      }))
    ...
    handleCallbackBatch(item) {
      ...
      this.$confirm('您确定手工回调这些订单吗?', '提示', {
        ...
        for (let i = 0; i < this.multipleSelection.length; i++) {
          this.doCallback({ orderNo: this.multipleSelection[i].order_no, isPay: 1 })
