  ...
  <div class="app-container">
    ...
    <el-table 
      ...
      <el-table-column label="所属渠道" align="center">
        <template slot-scope="scope">
          {{nameMap[scope.row.channel_code]}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="交易额" align="center" width="160">
        <template slot-scope="scope">
          {{$priceFormat((scope.row.max_money ? scope.row.max_money: 0).toFixed(2))}}(已用<span :class="scope.row.max_money == scope.row.money?'red':''">{{$priceFormat((scope.row.money ? scope.row.money: 0).toFixed(2))}}</span>)
        </template>
      </el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :title="titleMap[dialogStatus]" :visible.sync="createVisible" width="30%" v-draggable>
      <el-form class="small-space" :model="form" ref="form" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="所属渠道" prop="channel_code">
            <el-select v-if="dialogStatus=='create'" v-model="form.channel_code" clearable placeholder="全部" :loading="nameLoading">
                <el-option v-for="(value, key) in nameMap" :key="key" :label="value" :value="key">
                </el-option>
            </el-select>
            <el-select v-else v-model="form.channel_code" disabled placeholder="全部">
                <el-option v-for="(value, key) in nameMap" :key="key" :label="value" :value="key">
                </el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="通道名称" prop="way_name">
