<template>
  <div class="app-container">
    ...
    <el-table 
      ...
      <el-table-column prop="code_from_channel" label="分配商户ID" align="center" width="100"></el-table-column>
      <el-table-column label="今日充值额" align="center" width="160">
        <template slot-scope="scope">
      ...
      <el-table-column label="特殊渠道" align="center">
        <template slot-scope="scope">
          <span type="success" v-if="scope.row.special_way === 0">是</span>
          <span type="danger" v-else>不是</span>
        </template>
      </el-table-column>
      <el-table-column label="今日转账额" align="center" width="160">
        <template slot-scope="scope">
      ...
      <el-table-column label="收支差" align="center" width="160">
        <template slot-scope="scope">
          {{scope.row.diff_max ? $priceFormat(scope.row.diff_max.toFixed(2)): $priceFormat('0.00')}}(已用<span :class="scope.row.diff_max <= (scope.row.pay_today-scope.row.draw_today)?'red':''">{{(scope.row.pay_today-scope.row.draw_today) ? $priceFormat((scope.row.pay_today-scope.row.draw_today).toFixed(2)): $priceFormat('0.00')}}</span>)
        </template>
      </el-table-column>
      ...
      <el-table-column prop="pay_status" label="充值渠道状态" align="center" width="100">
        <template slot-scope="scope">
          ...
          <el-tag v-else size="mini" :type="scope.row.pay_status | statusFilter"> {{ statusMap[''+scope.row.pay_status] }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="draw_status" label="转账渠道状态" align="center" width="100">
        <template slot-scope="scope">
          ...
          <el-tag v-else size="mini" :type="scope.row.draw_status | statusFilter"> {{ statusMap[''+scope.row.draw_status] }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :title="titleMap[dialogStatus]" :visible.sync="createVisible" width="30%" v-draggable>
      <el-form class="small-space" :model="form" ref="form" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="特殊渠道" prop="special_way" size="small">
            <el-select v-model="form.special_way" clearable placeholder="请选择渠道类型">
...
export default {
    ...
    methods: {
        ...
        selectTypeChange(val){
            ...
            this.$set(this.queryParams, 'channel_name', null)
        },
        getMapList(data = {}) {
            ...
            listSelectChannel(data).then(response => {
                ...
                response.data.items.map(e => {
                    this.nameMap[''+e.channel_code] = e.channel_name
                })
