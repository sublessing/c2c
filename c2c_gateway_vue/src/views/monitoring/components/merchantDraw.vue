                ...
                for (var name in response.data.yAxisData) {
                  ...
                  this.seriesData.push({
                    ...
                    data: response.data.yAxisData[name].map(el => {
                      if (el && el.draw_rate !== undefined) {
                        el.value = el.draw_rate?(el.draw_rate*100).toFixed(2)*1:0;
                      }
                      return el;
                    })
                    // .map(el => (el.draw_rate?(el.draw_rate*100).toFixed(2)*1:0))
                  })
                }
                if (this.legendData.indexOf('全部商户') !== -1) {
                  this.legendData = [].concat(this.legendData.splice(this.legendData.indexOf('全部商户'), 1), this.legendData)
                }
