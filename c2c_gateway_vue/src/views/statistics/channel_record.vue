  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryParams" label-width="80px" size="mini">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="handleDownload" v-if="this.urls.includes('/statistics/channel_record/excel/')" :loading="downloadLoading">导出</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border stripe fit highlight-current-row size="small" @sort-change="handleSort">
      ...
      <el-table-column prop="channel_name" label="所属渠道" align="center">
      </el-table-column>
...
import { listChannelRecord, channelRecordExcel } from '@/api/stat'
...
export default {
  ...
  data() {
    return {
      ...
      channelMap: {},
      payWayMap: {'1':'支付宝', '2': '微信', '3':'银行卡', '4': '云闪付', '5': '线下交易'},
    }
  },
  created() {
    this.getChannelMap()
  ...
  methods: {
    ...
    fetchData() {
      ...
      listChannelRecord(this.queryParams).then(response => {
    ...
    handleDownload() {
      ...
      channelRecordExcel(this.queryParams).then(response => {
