...
import { jkAgentBalance } from '@/api/jk'
export default {
  data() {
    return {
      lastData: null,
  ...
  methods: {
    ...
    fetchDataHuiZong() {
      ...
      jkAgentBalance(this.queryParams).then(response => {
        this.seriesData = []
        ...
        this.seriesData.push({ name: '全部代理余额', type: 'line', data: y100Values })
        this.seriesData.push({ name: '外部代理余额', type: 'line', data: y1Values })
        this.seriesData.push({ name: '内部代理余额', type: 'line', data: y0Values })
