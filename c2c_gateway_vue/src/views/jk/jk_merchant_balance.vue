...
import { jkMerchantBalance } from '@/api/jk'
export default {
  data() {
    return {
      lastData: null,
  ...
  methods: {
    ...
    fetchDataHuiZong() {
      ...
      jkMerchantBalance(this.queryParams).then(response => {
        this.seriesData = []
        this.legendData = response.data.legendData
        this.xAxisData = response.data.xAxisData
        const y100Values = response.data.y100Values.map(v => {
          return v / 100
        })
        const y0Values = response.data.y0Values.map(v => {
          return v / 100
        })
        const y1Values = response.data.y1Values.map(v => {
          return v / 100
        })
        this.seriesData.push({ name: '全部商户余额', type: 'line', data: y100Values })
        this.seriesData.push({ name: '系统商户余额', type: 'line', data: y1Values })
        this.seriesData.push({ name: '普通商户余额', type: 'line', data: y0Values })
