  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryParams" label-width="80px" size="mini">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="handleDownload" v-if="this.urls.includes('/statistics/merchant_day/excel/')" :loading="downloadLoading">导出</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border stripe fit highlight-current-row size="small" @sort-change="handleSort">
      ...
      <el-table-column prop="order_draw_money" label="转账金额" align="center" sortable="custom">
        <template slot-scope="scope">
          {{ $priceFormat((scope.row.order_draw_money?scope.row.order_draw_money:0).toFixed(2)) }} | <span style="color: rgb(0, 204, 0);">{{$priceFormat((scope.row.order_draw_succ_money?scope.row.order_draw_succ_money:0).toFixed(2))}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="order_draw_num" label="转账笔数" align="center" sortable="custom">
        <template slot-scope="scope">{{ scope.row.order_draw_num }} | <span style="color: rgb(0, 204, 0);">{{scope.row.order_draw_succ_num||0}}</span></template>
      </el-table-column>
...
import { listDayMerchant, dayMerchantExcel } from '@/api/stat'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      listDayMerchant(this.queryParams).then(response => {
    ...
    handleDownload() {
      ...
      dayMerchantExcel(this.queryParams).then(response => {
        this.downloadData = response.data.items
        require.ensure([], () => {
          const { export_json_to_excel } = require('vendor/Export2Excel')
          const tHeader = [
            '统计日期', '商户名称', '当日余额', '收取手续费', 
            '充值金额', '充值笔数','充值成功率',
            '转账金额', '转账笔数','转账成功率'
          ]
          const filterVal = [
            'day', 'merchant_name', 'balance', 'pay_fee',
          ...
          export_json_to_excel(tHeader, data, '商户日报')
