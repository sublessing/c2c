    ...
    <el-dialog :title="titleMap[dialogStatus]" :visible.sync="createVisible" width="30%" v-draggable>
      <el-form class="small-space" :model="form" ref="form" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="渠道类型" prop="channel_type">
            <el-select v-model="form.channel_type" size="mini" clearable style="width:120px" placeholder="请选择渠道类型">
            <el-option v-for="(value, key) in channelMap" :key="key" :label="value" :value="key">
            </el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="渠道代码" prop="channel_code" v-if="dialogStatus =='create'">
          <el-input v-model="form.channel_code" placeholder="请输入渠道代码"></el-input>
        </el-form-item>
        <el-form-item label="最高允许收支差" prop="diff_max">
        ...
        <el-form-item label="适配器地址" prop='adapter'>
        ...
        <el-form-item label="充值token" prop='pay_key'>
          <el-input v-model="form.pay_key" placeholder="请输入token"></el-input>
        </el-form-item>
        <el-form-item label="提现token" prop='draw_key'>
          <el-input v-model="form.draw_key" placeholder="请输入token"></el-input>
        </el-form-item>
        <el-form-item label="渠道服务器地址" prop='allow_ip'>
...
import { listChannel, addChannel, editChannel, editChannelStatus, listSelectChannel } from '@/api/service'
export default {
    name: 'Channel',
    data(){
        return {
