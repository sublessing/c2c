  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryParams" size="mini">
      <el-form-item label="商户名称">
        <el-select v-model="queryParams.merchant_code" size="mini" clearable style="width:160px">
      ...
      <el-form-item label="子支付方式">
        <el-select v-model="queryParams.way_type_sub" size="mini" clearable style="width:160px">
    ...
    <el-table :cell-style="cellStyle" :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" @selection-change="handleSelectionChange"  @cell-click="selectItem" border stripe fit highlight-current-row size="small">
      ...
      <el-table-column label="商户名称" align="center" prop="merchant_code">
        <template slot-scope="scope">
          {{ merchantMap2[scope.row.merchant_code] }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="280" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="warning" @click.native="reload(scope.row)" size="mini">重载</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :close-on-click-modal="false" :title="titleMap[dialogStatus]" :visible.sync="createVisible" width="35%" v-draggable>
      <el-form class="small-space" :model="form" ref="form" :rules="rules" initial="off" label-position="left" label-width="100px">
        <el-form-item label="商户名称" prop="merchant_code">
          <el-select v-if="dialogStatus=='create'" v-model="form.merchant_code" size="small" clearable>
            <el-option v-for="(value, key) in merchantMap2" :key="key" :label="value" :value="key">
            </el-option>
          </el-select>
          <el-select v-else v-model="form.merchant_code" size="small" clearable disabled>
            <el-option v-for="(value, key) in merchantMap2" :key="key" :label="value" :value="value">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="支付方式" prop="way_type">
          <el-select v-model="form.way_type" size="small" clearable @change="wayChange">
            <el-option v-for="(value, key) in payMap" :key="key" :label="value" :value="key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="子支付方式" prop="way_type_sub">
          <el-select v-model="form.way_type_sub" size="small" clearable>
            <el-option v-for="(value, key) in supSelectWayMap" :key="key" :label="value" :value="key">
            </el-option>
          </el-select>
        </el-form-item>
...
import { listFictitiousTree, listFictitious, updateFictitious, addFictitious, reloadMerchantWay, deleteFictitious, changeFictitiousBalance, banFictitious } from '@/api/service'
...
export default {
  ...
  methods: {
    wayChange (i) {
      this.supSelectWayMap = {}
      for (var name in this.subWayMap) {
        if (name[0] == i) {
          this.$set(this.supSelectWayMap, name, this.subWayMap[name])
        }
      }
    },
    cellStyle (data) {
    ...
    deleteMerchantChannelBan(item) { 
      const cmsg = '即将删除商户通道『 '+this.merchantMap2[item.merchant_code]+' 』,请确认!'
      this.$confirm(cmsg, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
    ...
    reload(item) {
      this.$confirm('您确定重载『 '+this.merchantMap2[item.merchant_code]+' 』吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.reseting = true
        reloadMerchantWay({merchantCode: item.merchant_code, way_type_sub: item.way_type_sub}).then(response => {
          this.$notify({
