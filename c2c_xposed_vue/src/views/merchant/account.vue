...
<script>
import { listMerchant, updateMerchant, addMerchant, deleteMerchant, reloadMerchant, updateMerchantDayRechargeMax, updateMerchantDayDrawMax, banMerchant, banMerchantRechargeFlag, banMerchantDrawFlag, merchantAddMoney } from '@/api/service'
...
export default {
  data() {
    ...
    return {
      ...
      titleMap2: { 0: '批量修改充值限额', 1: '批量修改转账限额' },
  ...
  methods: {
    ...
    reload(item) {
      this.$confirm('您确定重载『 '+item.merchant_name+' 』吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.reseting = true
        reloadMerchant({merchantCode: item.merchant_code}).then(response => {
          this.$notify({
