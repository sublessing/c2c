  ...
  <div class="app-container">
    <!-- <el-form :inline="true" :model="queryParams" size="mini">
      ...
      <el-form-item label="代理">
        <el-select v-model="queryParams.agent_id" size="mini" style="width:160px" placeholder="请选择">
    ...
    <div id="cpb" style="width:680px; height:250px;" v-loading="listLoading">
    </div>
  </div>
</template>
...
import { channelBalance } from '@/api/service'
...
export default {
    ...
    methods: {
        ...
        fetchData(){
            ...
            channelBalance(this.queryParams).then(response => {
                this.seriesData = []
                this.legendData = [];
                ...
                for (var name in response.data.yAxisData) {
                  ...
                  for (var nameItem of response.data.yAxisData[''+name]) {
                    if (nameItem.name) {
                      seriesName = nameItem.name;
                      this.legendData.push(nameItem.name)
                      break;
                    }
                  }
                  this.seriesData.push({
                    name: seriesName,
                    type: 'line',
                    data: response.data.yAxisData[name].map(el => el.balance || 0)
                  })
                }
        ...
        intiChart(){
          var _self = this;
          this.chart = echarts.init(document.getElementById('cpb'))
          const option = {
            tooltip: {
              trigger: 'axis',
              axisPointer: {
              ...
              formatter: function(params, ticket, callback) {
                ...
                for (var i = 0, l = params.length; i < l; i++) {
                  res +=
                    '<br/>' + params[i].seriesName + ': ' + _self.$priceFormat(params[i].value.toFixed(2))
                }
                return res
              }
            },
