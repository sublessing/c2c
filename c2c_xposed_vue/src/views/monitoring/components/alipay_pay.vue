<template>
  <div class="app-container">
    <!-- <el-form :inline="true" :model="queryParams" size="mini">
      ...
      <el-form-item>
        <el-select v-model="queryParams.merchant_name" size="mini" multiple style="width:160px" placeholder="请选择">
    ...
    <div id="mpd" style="width:680px; height:250px;" v-loading="listLoading">
    </div>
  </div>
</template>
...
import { successRateMerchantDraw } from '@/api/service'
...
export default {
    ...
    methods: {
        ...
        fetchData(){
            ...
            successRateMerchantDraw(this.queryParams).then(response => {
                this.seriesData = []
                this.legendData = [];
                ...
                for (var name in response.data.yAxisData) {
                  ...
                  this.seriesData.push({
                    ...
                    data: response.data.yAxisData[name].map(el => {
                      el.value = el.rate_alipay_hour?(el.rate_alipay_hour*100).toFixed(2)*1:0;
                      return el;
                    })
                    // .map(el => (el.rate_alipay_hour?(el.rate_alipay_hour*100).toFixed(2)*1:0))
                  })
                }
        ...
        intiChart(){
          this.chart = echarts.init(document.getElementById('mpd'))
          const option = {
            tooltip: {
              trigger: 'axis',
              axisPointer: {
              ...
              formatter: function(params, ticket, callback) {
                ...
                for (var i = 0, l = params.length; i < l; i++) {
                  res +=
                    '<br/>' + params[i].seriesName + ': ' + (params[i].data.alipay_order_hour?params[i].data.alipay_order_hour:0) + ' | ' + (params[i].data.alipay_order_succ_hour?params[i].data.alipay_order_succ_hour:0) + ' （' + params[i].value.toFixed(2) + '%）'
                }
                return res
              }
            },
