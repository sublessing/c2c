<template>
  <div class="app-container">
    ...
    <div id="cpd" style="width:680px; height:250px;" v-loading="listLoading">
    </div>
  </div>
</template>
...
import { successRateChannelDraw } from '@/api/service'
...
export default {
    ...
    methods: {
        ...
        fetchData(){
            ...
            successRateChannelDraw(this.queryParams).then(response => {
                this.seriesData = []
                this.legendData = [];
                ...
                for (var name in response.data.yAxisData) {
                  ...
                  this.seriesData.push({
                    ...
                    data: response.data.yAxisData[name].map(el => {
                      el.value = el.rate_union_hour?(el.rate_union_hour*100).toFixed(2)*1:0;
                      return el;
                    })
                    // .map(el => (el.rate_union_hour?(el.rate_union_hour*100).toFixed(2)*1:0))
                  })
                }
        ...
        intiChart(){
          this.chart = echarts.init(document.getElementById('cpd'))
          const option = {
            tooltip: {
              trigger: 'axis',
              axisPointer: {
              ...
              formatter: function(params, ticket, callback) {
                ...
                for (var i = 0, l = params.length; i < l; i++) {
                  res +=
                    '<br/>' + params[i].seriesName + ': ' + (params[i].data.union_order_hour?params[i].data.union_order_hour:0) + ' | ' + (params[i].data.union_order_succ_hour?params[i].data.union_order_succ_hour:0) + ' （' + params[i].value.toFixed(2) + '%）'
                }
                return res
              }
            },
