                ...
                for (var name in response.data.yAxisData) {
                  ...
                  this.seriesData.push({
                    ...
                    data: response.data.yAxisData[name].map(el => {
                      el.value = el.rate_wx_hour?(el.rate_wx_hour*100).toFixed(2)*1:0;
                      return el;
                    })
                    // .map(el => (el.rate_wx_hour?(el.rate_wx_hour*100).toFixed(2)*1:0))
                  })
                }
            ...
            tooltip: {
              ...
              formatter: function(params, ticket, callback) {
                ...
                for (var i = 0, l = params.length; i < l; i++) {
                  res +=
                    '<br/>' + params[i].seriesName + ': ' + (params[i].data.wx_order_hour?params[i].data.wx_order_hour:0) + ' | ' + (params[i].data.wx_order_succ_hour?params[i].data.wx_order_succ_hour:0) + ' （' + params[i].value.toFixed(2) + '%）'
                }
                return res
              }
            },
