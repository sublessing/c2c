  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryParams" label-width="80px" size="mini">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="handleDownload" v-if="this.urls.includes('/statistics/way_day/excel/')" :loading="downloadLoading">导出</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border stripe fit highlight-current-row size="small" @sort-change="handleSort">
      ...
      <el-table-column prop="type" label="支付方式" align="center" sortable="custom">
        <template slot-scope="scope">{{ payMap[''+scope.row.type] }}</template>
      </el-table-column>
    ...
    handleDownload() {
      ...
      dayChannelExcel(this.queryParams).then(response => {
        ...
        require.ensure([], () => {
          ...
          const filterVal = [
            'day', 'type', 'order_money', 'order_num', 'pay_rate'
          ]
          const data = this.formatJson(filterVal, this.downloadData)
          export_json_to_excel(tHeader, data, '通道日报')
