  ...
  <div class="app-container">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit highlight-current-row size="small">
      ...
      <el-table-column align="center" label="操作" width="400" fixed="right" v-if="hasRole || urls.includes('/auth/user/curd/')">
        <template slot-scope="scope">
          ...
          <el-button v-if="!isAdmin(scope.row.id)" type="danger" @click.native="deleteUserBan(scope.row)" size="mini" icon="el-icon-delete">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="titleMap[dialogStatus]" :visible.sync="createVisible" width="30%" v-draggable>
      <el-form class="small-space" :model="form" ref="form" :rules="rules" label-position="left" label-width="80px">
        ...
        <el-form-item label="用户角色" prop="role" v-if="dialogStatus=='create'">
          <el-select v-model="form.role" size="small">
            <el-option v-for="(value, key) in roleMap" :key="key" :label="value" :value="key">
            </el-option>
          </el-select>
        </el-form-item>
...
import { delUser, listRoleSelect, listUser, getUser, addUser, updateUser, authUser, resetUserPassword, guanlianAgentAndMerchant } from '@/api/auth'
...
export default {
  ...
  methods: {
    getRoleList () {
      listRoleSelect().then(res => {
        res.data.items.map(el => {
          this.$set(this.roleMap, ''+el.id, el.name);
        })
      })
      .catch(err => {})
    },
    ...
    deleteUserBan(item) { 
      const cmsg = '即将删除账户『 '+item.username+' 』,请确认!'
      this.$confirm(cmsg, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
      }).then(() => {
        delUser({delete_id: item.id}).then(response => {
            this.$notify({
                title: '成功',
                message: '删除成功',
                type: 'success',
